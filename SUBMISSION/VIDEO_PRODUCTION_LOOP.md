...
This is the authoritative, low-reasoning workflow for the second cut. Never assemble the whole video by guessing at a global timeline.

## Fast path

```bash
# Only needed when narration text changes:
scripts/generate-narration-v2.sh

# Rebuild the deterministic cut:
scripts/build-submission-video-v2.sh

# Decode and contract-check the result:
scripts/verify-submission-video-v2.sh
```

Then open `SUBMISSION/final-video/pitt-devweek-submission-v2.mp4` and manually review the action anchors in `SUBMISSION/NARRATION_SYNC_PLAN.md`. A passing command-line check does not replace watching the edit.
...
2. **Verify words.** Transcribe the WAV with Whisper. Reject it if the transcript contains text from another segment or misses the segment's first sentence.
...
5. **Synchronize.** The visual transition must land within 0.35 seconds of its matching verb: `rises`, `passes`, `drops`, `refuels`, `zero`, or `closes`.
