...
work="${TMPDIR:-/tmp}/pitt-devweek-video-v2"
mkdir -p "$work"
...
require_cue() {
  [[ -s "SUBMISSION/audio/v2/$1.wav" ]] || {
    printf 'Missing narration cue %s. Run scripts/generate-narration-v2.sh first.\n' "$1" >&2
    exit 1
  }
}

# Opening and product context: picture follows the narration section, without fake interaction.
still SUBMISSION/video-assets/intro-1.png 5.5 "$work/01.mp4"
still SUBMISSION/video-assets/intro-2.png 7.0 "$work/02.mp4"
still SUBMISSION/screenshots/01-planning-context.png 10.5 "$work/03.mp4"
still SUBMISSION/screenshots/01-planning-context.png 17.5 "$work/04.mp4"
...
browser_hold "$r1" 17.0 2.6 "$o1" "$work/05g.mp4"
...
browser_hold "$r2" 22.0 1.6 "$o2" "$work/06i.mp4"
...
browser_hold "$r3" 18.0 13.2 "$o3" "$work/07h.mp4"
browser_clip "$r3" 19.0 1.0 "$o3" "$work/07i.mp4"
browser_hold "$r3" 20.0 2.4 "$o3" "$work/07j.mp4"
concat_video "$work/07.mp4" "$work"/07{a,b,c,d,e,f,g,h,i,j}.mp4

# Outcome 4: passes at 2.04 and 4.20 s, zero at 8.68 s, blocks at 10.02 s, closes at 12.36 s.
...
browser_hold "$r4" 21.2 2.8 "$o4" "$work/08h.mp4"
browser_clip "$r4" 22.2 1.0 "$o4" "$work/08i.mp4"
browser_hold "$r4" 23.0 6.9 "$o4" "$work/08j.mp4"
...
still "$work/codex-hold.png" 0.72 "$work/09b.mp4"
concat_video "$work/09.mp4" "$work/09a.mp4" "$work/09b.mp4"

still SUBMISSION/screenshots/04-report-and-machine-handoff.png 13.0 "$work/10.mp4"
still SUBMISSION/video-assets/outro.png 9.0 "$work/11.mp4"
...
durations=(5.5 7.0 10.5 17.5 13.0 16.0 32.0 19.5 24.0 13.0 9.0)
: > "$work/audio-concat.txt"
for i in "${!names[@]}"; do
  name="${names[$i]}"; duration="${durations[$i]}"
  require_cue "$name"
  ffmpeg -hide_banner -loglevel error -y -i "SUBMISSION/audio/v2/$name.wav" \
    -af "loudnorm=I=-18:TP=-1.5:LRA=7,apad,atrim=0:$duration" -ar 48000 -ac 1 -c:a pcm_s16le "$work/audio-$name.wav"
...
ffmpeg -hide_banner -loglevel error -y -i "$work/video.mp4" -i "$work/audio.wav" -i SUBMISSION/PITT_VIDEO_CAPTIONS_V2.srt \
  -map 0:v -map 1:a -map 2:0 -t 167.0 -c:v copy -c:a aac -b:a 192k -c:s mov_text \
  -metadata:s:s:0 language=eng -movflags +faststart SUBMISSION/final-video/pitt-devweek-submission-v2.mp4

ffmpeg -v error -i SUBMISSION/final-video/pitt-devweek-submission-v2.mp4 -f null -
duration="$(ffprobe -v error -show_entries format=duration -of default=nw=1:nk=1 SUBMISSION/final-video/pitt-devweek-submission-v2.mp4)"
...
ffprobe -v error -show_entries format=duration,size -show_entries stream=index,codec_name,codec_type:stream_tags=language -of json \
  SUBMISSION/final-video/pitt-devweek-submission-v2.mp4
